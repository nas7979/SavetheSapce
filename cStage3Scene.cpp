...
void cStage3Scene::Init()
{
	...
	Part->AddComponent<cTextRenderer>()->m_Text = "-----STAGE3-----";
	...
	OBJECT->AddObject<cBoss3>("Boss", Vec2(OFFSET + MAPX * 0.5, OFFSET + MAPY * 0.5), 0.5, Obj_Enemy);
	for (int i = 0; i < 2; i++)
	{
		...
		OBJECT->AddObject<cEnemy4>("Enemy4", Vec2(OFFSET + Random(100, MAPX - 100), OFFSET + Random(100, MAPY - 100)), 0.5, Obj_Enemy);
		OBJECT->AddObject<cEnemy4>("Enemy4", Vec2(OFFSET + Random(100, MAPX - 100), OFFSET + Random(100, MAPY - 100)), 0.5, Obj_Enemy);
	}
...
void cStage3Scene::Render()
{
	...
	IMAGE->Render(IMAGE->Find("Back3"), Vec2(-0, ((int)m_BackPos % 1800)), 0, Vec2(1, 1), 0.9, Vec2(0.5, 0.5));
	IMAGE->Render(IMAGE->Find("Back3"), Vec2(-0, ((int)m_BackPos % 1800) - 1800), 0, Vec2(1, 1), 0.9, Vec2(0.5, 0.5));
}

void cStage3Scene::RenderBack()
{
	if (SCENE->m_FlipBack)
	{
		IMAGE->Render(IMAGE->Find("Back3"), Vec2(-0, ((int)m_BackPos % 1800)), 0, Vec2(1, 1), 0.6974, Vec2(0.5, 0.5));
		IMAGE->Render(IMAGE->Find("Back3"), Vec2(-0, ((int)m_BackPos % 1800) - 1800), 0, Vec2(1, 1), 0.6974, Vec2(0.5, 0.5));
		return;
	}
